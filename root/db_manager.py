...
class DatabaseManager:
    ...
    async def get_active_users(self):
        async with aiosqlite.connect('main.db') as db:
            ex = await db.execute("""SELECT * FROM users""")
    ...
    async def add_log(self, userid, check_id, action, company: ""):
        async with aiosqlite.connect('main.db', timeout=20) as db:
            ex = await db.execute("""SELECT name, surname FROM users WHERE id == ?""", (userid,))
            user_info = await ex.fetchone()
            await db.execute("""INSERT INTO log VALUES(?, ?, ?, ?, ?, ?);""", (userid, user_info[0], user_info[1], check_id, action, company))
            await db.commit()
    ...
    async def get_available_checks(self, latitude1, longitude1, latitude2, longitude2):
        ...
        for company in companies:
            async with aiosqlite.connect('db/' + company, timeout=20) as db:
                ex = await db.execute(
                    """SELECT id, adress, payment FROM adress WHERE latitude BETWEEN (?) AND (?) AND longitude BETWEEN (?) AND (?) AND assigned = 0 AND done = 0""",
    ...
    async def get_user_checks(self, userid):
        ...
        for company in companies:
            async with aiosqlite.connect('db/' + company, timeout=20) as db:
                ex = await db.execute("""SELECT id, adress FROM adress WHERE assigned == ? and done != ?""", (userid, 1))
    ...
    async def get_check_company(self, check_id):
        companies = os.listdir("db")
        for company in companies:
            async with aiosqlite.connect('db/' + company, timeout=20) as db:
                ex = await db.execute("""SELECT id FROM adress WHERE id == ?""", (check_id,))
                db_check = await ex.fetchone()
                if db_check:
                    return company.split(".")[0]
    ...
    async def add_check(self, company, state):
        async with state.proxy() as data:
            if company == "sokolov":
                ...
                async with aiosqlite.connect('db/torti.db', timeout=20) as db:
                    await db.execute(
                        "INSERT INTO proverka VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        tuple(list(data.values())[1:]))
                    await db.execute("""UPDATE adress SET done == ? WHERE id == ?""", (1, data['number']))
                ...
                async with aiosqlite.connect('db/domix.db', timeout=20) as db:
                    await db.execute(
                        "INSERT INTO proverka VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        tuple(list(data.values())[1:]))
                    await db.execute("""UPDATE adress SET done == ? WHERE id == ?""", (1, data['number']))
                    await db.commit()
            elif company == "informat":
                async with aiosqlite.connect('db/informat.db', timeout=20) as db:
                    await db.execute(
                        "INSERT INTO proverka VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        tuple(list(data.values())[1:]))
                    await db.execute("""UPDATE adress SET done == ? WHERE id == ?""", (1, data['number']))
                ...
                async with aiosqlite.connect('db/lexmer.db', timeout=20) as db:
                    await db.execute(
                        "INSERT INTO proverka VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        tuple(list(data.values())[1:]))
                    await db.execute("""UPDATE adress SET done == ? WHERE id == ?""", (1, data['number']))
